...
async def detailuser(request, id_user: int):
    authentication = check_token(request)
    if(authentication[0]):
        authtoken = authentication[1]    
        data = request.json 
        ...
        if(isadmin):
            ...
            async with pool.acquire() as conn:
                sql = '''
                        SELECT * from user_person where id_user={0}; 
                    '''.format(id_user)
                rows = await conn.fetch(sql)
                return response.json({'status': 200, 'data': jsonify(rows)}, status=200)
        else:
...
def add_routes_user(app):
    ...
    app.add_route(alluser, '/user/alluser', methods=['GET'])        
    app.add_route(detailuser, '/user/<id_user:int>', methods=['GET'])        